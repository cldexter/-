    ...
    while tries > 0:
        try:
            ...
            response = requests.get(
                link, data=payload, headers=agents.get_header()).json()
            break
        except Exception, e:
            tries -= 1
    # 重新合成抓回来的数据
...
def save_game_data(league_name, host_team, guest_team, league_k, game_k, game_date_time, game_pass_time, game_half, host_team_score, guest_team_score, position_big, odd_id_big, odd_big, position_small, odd_id_small, odd_small):
    ...
    筛选offer交给函数，买不买交给对象'''
    game_record_new = {"add_time": ut.time_str(
        "full"), "league_name": league_name, "host_team": host_team, "guest_team": guest_team, "league_k": league_k, "game_k": game_k}
    game_status_new = {"last_update": ut.time_str("full"), "game_k": game_k, "game_past_time": game_pass_time, "game_half": game_half, "host_team_score": host_team_score, "guest_team_score": guest_team_score,
                       "position_big": position_big, "odd_id_big": odd_id_big, "odd_big": odd_big, "position_small": position_small, "odd_id_small": odd_id_small, "odd_small": odd_small}
    ...
    if not game_k in game_k_all:
        ...
        msg.msg(u"新加比赛", host_team + guest_team,
                u"添加", u"succ", "info", msg.display)
    else:
        mh.add_game_record(game_status_new, "status")
        msg.msg(u"已有比赛", host_team + guest_team,
                u"更新", "succ", "info", msg.display)
    ...
    for league in leagues:
        ...
        if league_filter(league_name):
            for game in league['es']:  # 对每一场比赛,进行如下操作
                ...
                if game_filter(host_team) and game_filter(guest_team):
                    ...
                    if host_team_score == "":  # 没有得分的，说明日期是比赛开始日期
                        game_date_time = ut.time_str_converter(
                            game_date, game_pass_time)
                    ...
                    if game_half_raw == "" and host_team_score != "":  # 比赛已经开始了
                        game_half = u"暂停"
                    elif game_half_raw == "" and host_team_score == "":
                        game_half = u"准备"
                    else:
                        game_half = ut.dict_replace(
                            game_half_raw, dic.dict_game_stage)
                    ...
                    if 'ou' in game['o'].keys():  # 如果有买大小的盘口的话
                        ...
                        for item in range(0, offer_number):  # 生成多个bet offer
                            # 买大盘口[1]
                            position_big = game['o']['ou'][i * 8 + 1]
                            # 买小盘口[3]和主队盘口应该完全一样
                            position_small = game['o']['ou'][i * 8 + 3]
                            # 买大赔率编号[4]
                            odd_id_big = game['o']['ou'][i * 8 + 4]
                            odd_big = game['o']['ou'][i * 8 + 5]  # 买大赔率[5]
                            # 买小赔率编号[6]
                            odd_id_small = game['o']['ou'][i * 8 + 6]
                            odd_small = game['o']['ou'][i * 8 + 7]  # 买小赔率[7]
                            # 买小（small_offer）是我们玩的主要东西
                            if offer_filter(position_big, odd_big, position_small, odd_small):
                                offer_small = (game_k, game_pass_time, game_half, host_team_score, guest_team_score,
                                               position_big, odd_id_big, odd_big, position_small, odd_id_small, odd_small)
                                game_offer_small_list.append(offer_small)
                            i += 1
                        if len(game_offer_small_list) > 0:
                            max_offer_small = sorted(game_offer_small_list, key=lambda offer_small: offer_small[5], reverse=True)[
                                0]  # 找到最大的offer_samll
                            save_game_data(league_name, host_team, guest_team, league_k, game_k, game_date_time, offer_small[1], offer_small[2], offer_small[
                                           3], offer_small[4], offer_small[5], offer_small[6], offer_small[7], offer_small[8], offer_small[9], offer_small[10])


def league_filter(league_name):
    '''在这里编写联赛过滤关键词'''
    key_words = [u"特别", u"分钟", u"点球"]
    for key_word in key_words:
        if key_word in league_name:
            return False
            break
    else:
        return True
...
def offer_filter(position_big, odd_big, position_small, odd_small):
    '''在这里输入offer过滤逻辑'''
